...
#include <xlfunction/math/xlCorrelationMatrixEWMA/xlCorrelationMatrixEWMA.hpp>

        /* fonction de calcul de la matrice de variance-covariance d'un historique de taux */
DLLEXPORT xloper * xlCorrelationMatrixEWMA(const xloper * seriesId_,
                                           const double * decay_,
...
boost::shared_ptr<ObjectHandler::FunctionCall> functionCall(
    new ObjectHandler::FunctionCall("xlCorrelationMatrixEWMA")) ;
     ...
     try {
        ...
        if (* norm_ == true) {
                ...
                for (unsigned long i = 0 ; i < timeSeriesVector.size() ; i++) {
                        ...
                        deltaSeriesVector.push_back(QuantLib::deltaTimeSeries<double>(timeSeriesVector[i])) ;
                ...
                for (std::vector<QuantLib::TimeSeries<double>>::const_iterator It = timeSeriesVector.begin() ;
                    It < timeSeriesVector.end() ; ++It) {
                        ...
                        mean.push_back(meanEWMA(* It, QuantLib::Date(static_cast<QuantLib::BigInteger>(* startDate_)), decay_)) ;
